...
export default function App() {
  ...
  // 连接到 Java 网关 WebSocket
  const { connect: wsConnect, disconnect: wsDisconnect, send: wsSend } = useWebSocket('ws://localhost:3000/ws/asr');
  ...
  const translateText = useCallback(async (text: string) => {
    ...
    try {
      ...
      // 通过网关 API 翻译
      const resp = await fetch(`/api/translate?text=${encodeURIComponent(text)}&sourceLang=${srcCode}&targetLang=${tgtCode}`, {
        method: 'POST',
  ...
  useEffect(() => {
    const handleMessage = (event: Event) => {
      ...
      if (msg.type === 'asr_final') {
        ...
        addConsoleLog('ok', `ASR final: "${msg.payload?.text || ''}"`);
        translateText(msg.payload?.text || '');
      }
      ...
      if (msg.type === 'subtitle_patch') {
        ...
        if (action === 'MARK_FINAL') {
          ...
          if (new_text) {
            ...
            addFinalResult(new_text, new_text);
          }
        }
      }
  ...
  }, [setStepState, addConsoleLog, showToast, addFinalResult, translateText]);
  ...
  const handleToggle = useCallback(async () => {
    ...
    addConsoleLog('info', 'LiveTranslate Console v1.0.0');
    addConsoleLog('info', `模式: ${mode === 'mic' ? '麦克风' : '标签页'}, 源: ${srcLang}, 目标: ${tgtLang}`);

    try {
      setStepState('init', 'active', '正在初始化...');
      setConnectionStatus('connecting');
      addConsoleLog('info', 'Connecting to ws://localhost:3000/ws/asr');
